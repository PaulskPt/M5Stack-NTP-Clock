 ...
 *     + -------------------------------------+ 240
 *     
 *     pos.x and pos.y values outside of touch area: -1, -1
 *     Max pos.x, pos.y seen: 317,276
  */

HotZone Btn0(0,   0, 320, 240, &switch_disp);  // upper half of the display
TouchPoint_t tp_pos;
...
void disp_frame()
{
    ...
    M5.Lcd.setCursor(30, vert[0]-10);
    M5.Lcd.print(TITLE);
    M5.Lcd.setCursor(0, vert[1]+5);
    M5.Lcd.print("Day: ");
    M5.Lcd.setCursor(0, vert[2]+5);
    M5.Lcd.print("Date: ");
    M5.Lcd.setCursor(0, vert[3]+5);
    M5.Lcd.print("Time: ");
    M5.Lcd.setCursor(0, vert[4]+5);
    M5.Lcd.print("Time-Zone: ");
}
...
void switch_disp()
{
  if(tb_state)  // determine if touch inside this hot zone
  {
    if (disp_state)
    {
